...
//
// Schema is never applied automatically. NewPostgresCache only pings the
// pool — the two tables it queries (grcache_entries, grcache_entry_tags)
// must already exist, applied through your own project's migration tool
// (golang-migrate, Flyway, a plain SQL file run in CI, whatever you
// already use) before ever constructing a Cache. See PostgresSchemaSQL and
// docs/architecture.md's "GORM removed" section. This mirrors the fix
// gourdiantoken shipped in its own v2.4.0: auto-applying schema on every
// connect requires the connecting role to have CREATE on the target
...
// with a DML-only role) won't have — and that failure can't be worked
// around by pre-creating the tables some other way, since CREATE TABLE IF
// NOT EXISTS still checks CREATE privilege before checking whether the
// table exists.
package grcache
...
// be returned by PostgresSchemaSQL() and applied by grcache's own test
// setup. NewPostgresCache itself never applies this — see
...
// PostgresSchemaSQL returns grcache's Postgres cache schema (plain CREATE
// TABLE/INDEX IF NOT EXISTS, no grcache-specific magic) as text, for the
// caller to apply through their own project's migration tool
// (golang-migrate, Flyway, a plain SQL file run in CI, whatever you
// already use) before constructing NewPostgresCache.
//
// grcache deliberately never applies its own schema at runtime: doing so
...
// grcache and its schema changes. See docs/architecture.md's "GORM
// removed" section for the full pattern.
...
// statements are not fully race-free in Postgres. Advisory locks are
// global to the database, not namespaced per application, so an unrelated
// app that happens to choose this same int64 key could in principle
// collide with it — an accepted, extremely unlikely risk, not worth a
// namespacing scheme for a single fixed lock. Distinct from
...
// NewPostgresCache opens a connection pool per cfg and validates
// connectivity via Ping before returning.
//
// This never applies grcache's schema itself — see PostgresSchemaSQL. The
// two tables it queries (grcache_entries, grcache_entry_tags) must already
// exist before this is called, applied through your own project's
// migration tool. Calling this against a database where they don't exist
// yet doesn't fail here — it fails on the first actual cache operation
// (Get, Set, ...) with a plain Postgres "relation does not exist" error,
// wrapped as ErrCacheUnavailable.
//
// Parameters:
...
// Returns:
//   - Cache: ready to use
//   - error: non-nil if DSN is empty or malformed, or the connection/Ping
//     fails; connection/Ping failures wrap ErrCacheUnavailable
//
// Example:
//
