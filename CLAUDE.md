...
Once, create the Postgres test database if `make docker-up` hasn't already: `createdb -U postgres_user -h localhost grcache_test` (or `psql ... -c "CREATE DATABASE grcache_test;"`). Redis and Mongo need no equivalent setup step — Redis DB 14 and the Mongo `grcache_test` database are created implicitly on first write. The Postgres *schema* (`grcache_entries`/`grcache_entry_tags` tables, distinct from the database itself) also needs no manual step to run this repo's own tests — `NewPostgresCache` itself no longer applies schema (see `postgres.go`'s `PostgresSchemaSQL()`), but `postgres_test.go`'s own test helpers (`newPostgresCache`/`truncatePostgresTestDB`) apply it via `PostgresSchemaSQL()` before every run. A real consumer must apply `PostgresSchemaSQL()` through their own migration tool before constructing `NewPostgresCache` — see `docs/architecture.md`'s "GORM removed" section.
